...
async def update_sites(api, siteId: str = None, fromFile: bool = False) -> dict:  # noqa: C901
        ...
        if myid:
            ...
            for index, solarbank in enumerate(sb_list):
                ...
                if sn:
                    ...
                    schedule = (api.devices.get(sn, {})).get("schedule")
                    if schedule:
                        api._update_dev(
                            {
                                "device_sn": sn,
...
async def update_site_details(
    api, fromFile: bool = False, exclude: set = None
) -> dict:
...
async def update_device_details(
    ...
    for sn, device in api.devices.items():
        ...
        if device.get("is_admin", False) and site_id:
            ...
            wifi_index = device.get("wireless_type", "")
            if (
                not wifi_index
                and len(wifi_list) == 1
...
async def update_device_energy(
    ...
    for site_id, site in api.sites.items():
        ...
        if (
            ...
            sn = dev_list[0].get("device_sn")
            if sn:
                query_types |= {SolixDeviceType.INVERTER.value}
                query_sn = sn
        sn = (site.get("grid_info") or {}).get("device_sn")
        if sn:
            query_types -= {SolixDeviceType.INVERTER.value}
            if not (
                {
