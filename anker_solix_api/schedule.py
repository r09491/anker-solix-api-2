...
async def get_device_load(
    ...
    for slot in schedule.get("ranges") or []:
        for dev in slot.get("device_power_loads") or []:
            ...
            if sn and sn not in dev_serials:
...
async def set_home_load(  # noqa: C901
    ...
    test_schedule: dict = None,  # used only for testing instead of real schedule
    test_count: int
    = None,  # used only for testing instead of real solarbank count
):
    ...
    charge_prio = (
        int(charge_prio)
        if str(charge_prio).isdigit() or isinstance(charge_prio, int) or isinstance(charge_prio, float)
        else None
    )
    preset = (
        int(preset)
        if str(preset).isdigit() or isinstance(preset, int) or isinstance(preset, float)
        else None
    )
    dev_preset = (
        int(dev_preset)
        if str(dev_preset).isdigit() or isinstance(dev_preset, int) or isinstance(dev_preset, float)
        else None
    )
    if (
        preset is None
    ...
    if test_schedule is not None:
        ...
        schedule = (self.devices.get(deviceSn) or {}).get("schedule") or {}
        if schedule:
    ...
    min_load = str(schedule.get("min_load"))
    if min_load.isdigit():
        ...
        min_load = SolixDefaults.PRESET_MIN
    max_load = str(schedule.get("max_load"))
    if max_load.isdigit():
    ...
    min_load_dev = str(schedule.get("advanced_mode_min_load"))
    if min_load_dev.isdigit():
...
async def set_sb2_home_load(  # noqa: C901
    ...
    preset: int = None,
    usage_mode: int = None,
    set_slot: Solarbank2Timeslot = None,
    insert_slot: Solarbank2Timeslot = None,
    test_schedule: dict = None,  # used only for testing instead of real schedule
):
    ...
    if preset is not None or set_slot or insert_slot:
        for idx in rate_plan:
            days = set(idx.get("week") or [])
            if len((days) & weekdays) > len(
