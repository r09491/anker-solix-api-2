...
def requestDelay(self, delay: float = None) -> float:
    ...
    if (
        delay is not None
        and (isinstance(delay, float) or isinstance(delay, int))
...
async def _wait_delay(self, delay: float = None) -> None:
    ...
    if delay is not None and (isinstance(delay, float) or isinstance(delay, int)):
        delay = float(
            min(
                SolixDefaults.REQUEST_DELAY_MAX,
                max(SolixDefaults.REQUEST_DELAY_MIN, delay),
...
async def request(
    ...
    *,
    headers: dict = None,
    json: dict = None,  # pylint: disable=redefined-outer-name
) -> dict:
