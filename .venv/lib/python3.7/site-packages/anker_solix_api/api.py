...
class AnkerSolixApi:
    ...
    def __init__(
        ...
        self._gtoken: str = None
        self._token: str = None
        self._token_expiration: datetime = None
    ...
    async def _saveToFile(self, filename: str, data: dict = None) -> bool:
            ...
            for key, value in devData.items():
                try:
                    ...
                    # Add solarbank metrics depending on device type
                        ...
                        ##):
                        if device.get("device_pn") == "A17C0":
                            device.update({key: str(value)})
                        else:
                            print("Hello1", device.get("device_pn"), value)
                    # solarbank info shows the load preset per device, which is identical to device parallel_home_load for 2 solarbanks, or current homeload for single solarbank
                        ...
                        if device.get("device_pn") == "A17C0":
                            ...
                            print("Hello2", device.get("device_pn"), value)
                    # solarbank info shows the load preset per device, which is identical to device parallel_home_load for 2 solarbanks, or current homeload for single solarbank
                    ...
                    # When individual device schedules are supported in future, this info is needed per device anyway
                        ...
                        if not devData.get("retain_load"):
                            mysite = self.sites.get(device.get("site_id") or "") or {}
                            if mysite and sys_power:
    ...
    async def update_sites(
        self, siteId: str = None, fromFile: bool = False
    ) -> dict:  # noqa: C901
    ...
    async def set_auto_upgrade(self, devices: dict[str, bool]) -> bool:
        ...
        main_switch = settings.get("main_switch")
        if main_switch is None:
            return False
        dev_switches = {}
        main = None
        change_list = []
        for dev_setting in settings.get("device_list") or []:
            if isinstance(dev_setting, dict):
                device_sn = dev_setting.get("device_sn")
                if device_sn:
                    dev_upgrade = dev_setting.get("auto_upgrade")
                    if dev_upgrade is not None:
